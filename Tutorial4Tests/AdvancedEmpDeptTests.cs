...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, emp => emp.Mgr, mgr => mgr.EmpNo, 
            (emp, mgr) => new { Employee = emp.EName, Manager = mgr.EName }).ToList();
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps.Select(emp => new { emp.EName, Total = emp.Sal + (emp.Comm ?? 0)}).ToList(); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            .Join(depts, 
                emp => emp.DeptNo, 
                dept => dept.DeptNo, 
                (emp, dept) => new { emp.EName, dept.DName, emp.Sal })
            .SelectMany(ed => grades
                    .Where(grade => ed.Sal >= grade.Losal && ed.Sal <= grade.Hisal),
                (ed, grade) => new { ed.EName, ed.DName, ed.Sal, grade.Grade })
            .ToList();
