...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var chicagoDepts = depts.Where(dept => dept.Loc == "CHICAGO").Select(dept => dept.DeptNo);
    ...
    public void ShouldSelectNamesAndSalaries()
    {
        ...
        var result = emps.Select(emp => new { emp.EName, emp.Sal }).ToList();
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo)
            .Select(grp => new { DeptNo = grp.Key, Count = grp.Count() }).ToList(); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(emp => emp.Comm != null)
            .Select(emp => new { emp.EName, emp.Comm }).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps
            .SelectMany(emp => grades
                ...
                (emp, grade) => new { emp.EName, grade.Grade })
            .ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(grp => 
            new { DeptNo = grp.Key, AvgSal = grp.Average(emp => emp.Sal) }).ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var averages = emps.GroupBy(emp => emp.DeptNo).Select(group 
            => new { DeptNo = group.Key, AvgSal = group.Average(emp => emp.Sal) }).ToDictionary(g => g.DeptNo, 
            g => g.AvgSal);
        
        var result = emps.Where(emp => emp.Sal > averages[emp.DeptNo]).Select(emp => emp.EName).ToList();
